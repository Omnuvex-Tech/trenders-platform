...
export async function VacancyDetailWrapper({
  ...
  const responsible = vacancy.responsible?.map((item) => getL(item, locale)).filter(Boolean);
  ...
  const requirements = vacancy.requirements?.map((item) => getL(item, locale)).filter(Boolean);
