...
// Köhnə (flat string[] və ya tək string) formatları da dəstəkləyir. Aktiv
// dildə şəkil yoxdursa AZ -> EN -> RU sırası ilə fallback edir — bu fallback
// YALNIZ canlı saytın render tərəfində tətbiq olunur, admin formada yox.
...
function renderSection(section: any, index: number, blog: any, locale: string) {
    ...
    switch (section.type) {
        ...
        case 'content': {
            const heroImage = toAbsUrl(resolveLocalizedImage(section.heroImage, locale));
            const bottomLeft = toAbsUrl(resolveLocalizedImage(section.bottomImages?.left, locale));
            const bottomRight = toAbsUrl(resolveLocalizedImage(section.bottomImages?.right, locale));
            return (
                <BlogDetailContentUI
                    key={index}
                    heroImage={heroImage}
        ...
        case 'article': {
            return (
                <BlogDetailArticleUI
                    key={index}
                    sections={(section.sections ?? []).map((s: any) => {
                        const sideImage = resolveLocalizedImage(s.sideImage, locale);
                        return {
                            blocks: (s.blocks ?? []).map((b: any) => ({
                                type: b.type,
                                content: t(b.content, locale),
                            ...
                            sideImage: sideImage ? toAbsUrl(sideImage) : undefined,
                            sideImageAlt: t(s.sideImageAlt, locale),
                        };
                    })}
                    hashtags={
